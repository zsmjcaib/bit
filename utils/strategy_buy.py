        ...
        if (line.iloc[i]["key"] < low and line.iloc[i]["flag"] == "down") or \
            ...
            if line.iloc[i]["key"] < low and line.iloc[i]["flag"] == "down":
                ...
                if flag > 1 :
                    ...
                    return flag, line["key"].iloc[-1],result,'no','',line["key"].iloc[-1]
                else:
...
def strategy_test_buy(test_15_simple,test_15,test_15_deal,test_15_line,test_1h,test_1h_deal,test_1h_line,test_4h,test_4h_deal,test_4h_line):
    l = pd.DataFrame({'date':'','gear':'yes','15m':'','1h':'','15m小转大':'','1h小转大':'','flag':'','loss':'','point':'','is_grid':'','grid':'','sl':'','direction':''}, index=[1])
...
def calculate(low,low_deal,low_line,test_1h,high,test_1h_line):
    l = pd.DataFrame({'date':'','gear':'','15m':'','1h':'','15m小转大':'','1h小转大':'','flag':'','loss':'','point':'','is_grid':'','grid':'','sl':'','direction':''}, index=[1])
    result = 'no'
    mark_price =0
